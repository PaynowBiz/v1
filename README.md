# PaynowBiz API
* [테스트 화면](https://paynowbiz.tosspayments.com/sample/v1View.do) 에서 테스트를 할 수 있습니다.
...
<br/><br/>
## ※ 주의사항 ※
* `app2appyn` servicecode in (member, customer) 인 경우만 해당하며, Y로 할 경우 영업사원ID에 initialname 이 붙으므로, 가맹점에서 연동방식을 정확히 확인 후 넘겨야 합니다.
...
|`reqid`|필수|17|숫자|yyyyMMddHHmmssSSS|
|`startdt`|필수|8|숫자|시작일[YYYYMMDD] <br/>조회기준 : 거래내역(**`paydate`** or **`canceldate`**), 정산내역(**`adjustdate`**)|
|`enddt`|필수|8|숫자|종료일[YYYYMMDD] <br/>조회기준 : 거래내역(**`paydate`** or **`canceldate`**), 정산내역(**`adjustdate`**)|
